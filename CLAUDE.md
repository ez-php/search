...
**`ElasticsearchDriver`** — `flush()` on an index that does not exist yet and `remove()` of a missing document are no-ops (404 swallowed), matching the other drivers; `flush()` uses `refresh=true` so the deletion is visible to the next search.
...
Integration tests for Elasticsearch (`#[Group('elasticsearch')]`) and Typesense (`#[Group('typesense')]`) need running servers, which are **not** part of the default local stack (Elasticsearch alone needs ~1 GB RAM). They live in the opt-in override `docker-compose.ci.yml` (no published host ports — reached as `elasticsearch:9200` / `typesense:8108` over the Compose network):

- **CI** runs them for real: the `test` job sets `COMPOSE_FILE=docker-compose.yml:docker-compose.ci.yml` and waits for both health checks before PHPUnit.
- **Locally**, run `docker compose -f docker-compose.yml -f docker-compose.ci.yml up -d` in this module, or point `ELASTICSEARCH_HOST` / `TYPESENSE_HOST` (+ `TYPESENSE_KEY`) at real servers.
- **Monorepo root suite**: neither service exists there, so both classes skip. The first failed probe per class is cached (`self::$unreachable`), so the remaining tests skip instantly instead of each waiting out a connect timeout.

In the standalone Docker environment:
- `meilisearch` service is defined in `docker-compose.yml` — container port 7700, published on host port **7701**
...
|---|---|---|
| Meilisearch | Integration tests | 7700 in-container, 7701 on the host |
